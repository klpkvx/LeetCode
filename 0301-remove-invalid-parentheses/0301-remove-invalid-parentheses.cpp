...
public:
    ...
    void dfs (string &s, int index, int left_removed, int right_removed, int pair, string path, unordered_set<string> &result)
    {
        if (index >= s.size ()){
            ...
            if (s[index] == '('){
                ...
                dfs (s, index + 1, left_removed, right_removed, pair + 1, path + s[index], result);
            }
            else if (s[index] == ')'){
                if (right_removed > 0)
                ...
                dfs (s, index + 1, left_removed, right_removed, pair, path + s[index], result);
        }
    }
    vector<string> removeInvalidParentheses(string s) {
        ...
        dfs (s, 0, left_removed, right_removed, 0, "", result);
